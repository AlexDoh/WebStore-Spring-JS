    ...
    <div class="row">
        <nav class="col-sm-3 col-md-2 d-none d-sm-block bg-light sidebar">
            <ul class="nav nav-pills flex-column">
                <li class="nav-item">
                    <a class="nav-link active" href="#">${product.name} <span class="sr-only">(current)</span></a>
                </li>
                <c:forEach var="p" items="${category.productList}">
                    <li class="nav-item">
                        <a class="nav-link"
                           href="${pageContext.request.contextPath}/product?id=${p.id}"
                           style="font-size: 20px">${p.name}</a>
                    </li>
                </c:forEach>
            </ul>
        </nav>
        ...
        <main class="col-sm-9 ml-sm-auto col-md-10 pt-3" role="main">
            ...
            <section class="row text-center placeholders">
                <div class="col-6 col-sm-3 placeholder">
                    <a class="nav-link" href="${pageContext.request.contextPath}/product?id=${product.id}"
                       style="font-size: 20px">
                        <img src="data:image/gif;base64,R0lGODlhAQABAIABAAJ12AAAACwAAAAAAQABAAACAkQBADs="
                             width="400"
                             height="400" class="img-fluid rounded-circle" alt="Generic placeholder thumbnail"
                             style="border:0">
